...
import (
	...
	"github.com/ofbank_wallet/OFBANK_WALLET/internal/jsre"
	"github.com/ofbank_wallet/OFBANK_WALLET/internal/web3ext"
...
func (c *Console) init(preload []string) error {
	...
	ofbank, err := c.jsre.Get("ofbank")
	if err != nil {
		return err
	}

	if obj := ofbank.Object(); obj != nil{

		if _, err = c.jsre.Run(`jeth.register = ofbank.register;`); err != nil {
			return fmt.Errorf("ofbank.register: %v", err)
		}

		if _, err = c.jsre.Run(`jeth.unlockW = ofbank.unlockW;`); err != nil {
			return fmt.Errorf("ofbank.pass: %v", err)
		}
		if _, err = c.jsre.Run(`jeth.registerWithMulKeys = ofbank.registerWithMulKeys;`); err != nil {
			return fmt.Errorf("ofbank.registerWithMulKeys: %v", err)
		}


		obj.Set("register", bridge.Register)
		obj.Set("registerWithMulKeys", bridge.RegisterWithMulKey)
		obj.Set("unlockW", bridge.Pass)
...
func (c *Console) Welcome() {
	...
	fmt.Fprintf(c.printer, "Welcome to the OFBank JavaScript console!\n\n")
	c.jsre.Run(`
