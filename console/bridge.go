...
//nancy
func (b *bridge) Pass(call otto.FunctionCall) (response otto.Value) {
	...
	val, err := call.Otto.Call("jeth.unlockW", nil, account, passwd, duration)
	if err != nil {
		throwJSException(err.Error())
	}
	return val
...
//Nancy
func (b *bridge) Register(call otto.FunctionCall) (response otto.Value) {
	var (

		ccode = call.Argument(0)
		password  string
		confirm  string
		err      error
	)
	...
	if !ccode.IsString() {
		throwJSException("first argument must be the contry code ")
	}
	switch {
	// No password was specified, prompt the user for it
	case len(call.ArgumentList) == 1:
		...
		// A single string password was specified, use that
	case len(call.ArgumentList) == 2 && call.Argument(1).IsString():
	...
	// Password acquired, execute the call and return
	ret, err := call.Otto.Call("jeth.register",nil,ccode, password)
	if err != nil {
		throwJSException(err.Error())
	}
	return ret
...
//Nancy
func (b *bridge) RegisterWithMulKey(call otto.FunctionCall) (response otto.Value) {
	var (

		ccode = call.Argument(0)
		count = call.Argument(1)
		password  string
		confirm  string
		err      error
	)
	...
	case len(call.ArgumentList) == 2:
		...
		// A single string password was specified, use that
	case len(call.ArgumentList) == 3 && call.Argument(2).IsString():
		password, _ = call.Argument(2).ToString()
	...
	ret, err := call.Otto.Call("jeth.registerWithMulKeys",nil,ccode,count,password)
	if err != nil {
		throwJSException(err.Error())
	}
	return ret
...
func (b *bridge) Send(call otto.FunctionCall) (response otto.Value) {
	...
	for _, req := range reqs {
		resp, _ := call.Otto.Object(`({"jsonrpc":"3.0"})`)
		resp.Set("id", req.Id)
